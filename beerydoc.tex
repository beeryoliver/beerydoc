...
\begin{code}{\key}
  \begin{syntax}
    \cs{key} \oarg{key-value list} \marg*{\meta{key}=\meta{value}}
    \cs{key} \oarg{key-value list} \marg{key}
    \cs{key} \oarg{key-value list} \marg*{=\meta{value}}
  \end{syntax}
  ...
  \end{itemize}
  \meta{key-value list} sets the keys in path |key| for only that instance of
  \cs{key}.
\end{code}
