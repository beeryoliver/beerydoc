...
Version 0.0.0\quad 28 January 2026
...
\begin{code}{\meta}
  ...
  \listheading
    {
      If the value of the key \key{meta/replace-expr} is \key{=true}
      (\S\ref{sec:keys}) and \meta{meta} begins with the tokens on the left,
      then they will be replaced by the tokens on the right:
    }
  \begin{itemize}[parsep=0pt]
...
This section documents the keys provided by the \cls{beerydoc} class.

\begin{code}{frenchspacing}
  \begin{syntax}
    \key{frenchspacing=\choices{true,false}}
  \end{syntax}
  Boolean key that sets whether \cs{frenchspacing} (true) or
  \cs{nonfrenchspacing} (false) is enabled.
  The initial value is \key{=true}.
\end{code}

\begin{code}{meta/replace-expr}
  \begin{syntax}
    \key{meta/replace-expr=\choices{true,false}}
  \end{syntax}
  Boolean key that sets whether to replace shorthands for various
  computational expressions with their longer form.
  See the documentation for \cs{meta} for details (\S\ref{subsec:clsmarkup}).
  The initial value is \key{=true}.
\end{code}
